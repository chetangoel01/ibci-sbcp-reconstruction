"""Train improved Transformer on Modal with A10G GPU.

Improvements over v1:
  - Data augmentation (gain noise, channel dropout, Gaussian noise)
  - Velocity auxiliary loss (weight=0.1)

Usage:
    modal run modal_train_transformer_v2.py
"""

import modal

app = modal.App("phase2-transformer-ctx400")

data_vol = modal.Volume.from_name("phase2-data", create_if_missing=True)
output_vol = modal.Volume.from_name("phase2-outputs-ctx400", create_if_missing=True)
...
def train_transformer_v2():
    ...
    train_fn(config)
    output_vol.commit()
    print("Transformer v2 training complete.")
...
def main():
    print("Starting Transformer v2 training on A10G (augmentation + velocity loss)...")
    train_transformer_v2.remote()
    print("Training done. Running inference...")
    run_inference.remote()
    print("All done!")
    print("Download results:")
    print("  modal volume get phase2-outputs-ctx400 checkpoints/best_transformer.pt .")
    print("  modal volume get phase2-outputs-ctx400 results/submission_transformer.csv .")
